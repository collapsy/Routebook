...
test("hospedagem é localizada ao salvar e habilita contexto espacial", async ({
  ...
  const email = `${suffix}@example.com`;
  const password = "routebook-e2e-password";
  const tripName = `Floripa hospedagem ${suffix}`;

  await page.goto("/criar-conta?next=%2Fviagens");
  await page.getByLabel("Nome").fill("Owner RB-INC-181");
  await page.getByLabel("Email").fill(email);
  await page.getByLabel("Senha").fill(password);
  await page.getByRole("button", { name: "Criar conta" }).click();
  await expect(page).toHaveURL(/\/viagens$/);
  ...
  await page.getByLabel("Endereço").fill("Av. Beira-Mar Norte, 1000");
  ...
  await expect(page.getByRole("heading", { name: /Mapa do entorno de Florianópolis/ })).toBeVisible();
  await expect(page.getByLabel("Resumo do mapa")).toContainText("4 pontos representados");
  await expect(page.getByLabel("Resumo do mapa")).toContainText("Descoberta externa");
