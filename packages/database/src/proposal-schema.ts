  ...
  (table) => [
    ...
    check(
      ...
      sql`(
        ...
        OR (${table.status} = 'partially-accepted' AND ${table.generationStartedAt} IS NOT NULL AND ${table.generationMethod} IS NOT NULL AND ${table.generationVersion} IS NOT NULL AND ${table.contentSchemaVersion} = 1 AND ${table.criteria} IS NOT NULL AND ${table.justifications} IS NOT NULL AND ${table.limitations} IS NOT NULL AND ${table.planningConflictIds} IS NOT NULL AND ${table.generatedAt} IS NOT NULL AND ${table.validUntil} IS NOT NULL AND ${table.acceptedAt} IS NOT NULL AND ${table.rejectedAt} IS NULL AND ${table.expiredAt} IS NULL AND ${table.failedAt} IS NULL AND ${table.failureCode} IS NULL AND ${table.cancelledAt} IS NULL AND ${table.updatedAt} = ${table.acceptedAt})
