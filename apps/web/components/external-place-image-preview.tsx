...
export function ExternalPlaceImagePreview({
  ...
  const hasGovernedSource = Boolean(destinationId || googlePlaceId);

  useEffect(() => {
    const target = containerRef.current;
    if (!target || state.status !== "idle" || !hasGovernedSource || !enabled) return;
  ...
  }, [enabled, hasGovernedSource, state.status]);

  useEffect(() => {
    if (state.status !== "loading" || !hasGovernedSource || !enabled) return;
            ...
            <figcaption className={styles.caption}>
              ...
              {preview.authorAttributions.map((attribution) =>
                attribution.uri ? (
                  ...
                  <span key={attribution.displayName}>{attribution.displayName}</span>
                ),
              )}
              {preview.sourceUrl ? (
