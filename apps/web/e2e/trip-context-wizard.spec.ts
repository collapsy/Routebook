...
test("preenche Contexto progressivamente sem criar Activity ou Proposal", async ({ page }) => {
  ...
  await expect(page.getByRole("definition").filter({ hasText: trip.destination.name })).toHaveCount(
    1,
  );
  ...
  await Promise.all([
    page.waitForURL(
      new RegExp(
        `/viagens/${trip.id}/contexto\\?preparar=1&grupo=logistica&salvo=1import { expect, test } from "@playwright/test";
