...
test("preenche Contexto progressivamente sem criar Activity ou Proposal", async ({ page }) => {
  ...
  await expect(
    page.getByRole("link", { name: "Continuar para Contexto" }),
  ).toHaveAttribute("href", `/viagens/${trip.id}/contexto?preparar=1`);
  ...
  await page.getByRole("link", { name: /Lugares/ }).first().click();
  await expect(page).toHaveURL(
    new RegExp(`/viagens/${trip.id}/lugares-salvos\\?preparar=1$`),
  );
