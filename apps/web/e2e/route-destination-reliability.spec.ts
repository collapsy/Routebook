...
test("usa nome e endereço nos destinos individuais do catálogo, detalhe e Guia", async ({
  ...
  const cardRouteHref = await praiaDoAmorCard
    ...
    .getAttribute("href");
  expect(googleMapsDestination(cardRouteHref)).toBe(
  ...
  const firstGuideRoute = page.getByRole("link", { name: "Rota e tempo no Maps" }).first();
  await expect(firstGuideRoute).toBeVisible();
  const guideHref = await firstGuideRoute.getAttribute("href");
