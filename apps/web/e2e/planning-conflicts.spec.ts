...
test("revisa um conflito de horários e retorna ao dia afetado", async ({ page }, testInfo) => {
  ...
  await expect(affectedDayLink).toHaveAttribute("href", /\/roteiro\?dia=2026-08-22#dia-em-foco$/);
...
test("ignora um risco e preserva seu histórico auditável", async ({ page }, testInfo) => {
  ...
  await expect(backToItinerary).toHaveAttribute("href", new RegExp(`/viagens/${tripId}/roteiro$`));
