...
--> statement-breakpoint
ALTER TABLE "itinerary_proposals" ADD COLUMN "rejected_at" timestamp with time zone;
--> statement-breakpoint
ALTER TABLE "itinerary_proposals" ADD CONSTRAINT "itinerary_proposals_status_check"
CHECK ("status" IN ('requested', 'generating', 'ready', 'rejected', 'expired', 'failed', 'cancelled'));
--> statement-breakpoint
...
CHECK (
  "status" NOT IN ('ready', 'rejected', 'expired')
  OR (
