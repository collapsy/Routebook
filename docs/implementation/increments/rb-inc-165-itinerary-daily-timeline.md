...
Os três E2E adicionais são indispensáveis porque dependiam diretamente do markup/hierarquia substituídos pelo Roteiro diário: `itinerary-spatial.spec.ts` valida o painel espacial compacto; `place-actions.spec.ts` valida a entrada de Place na timeline; e `recommendations-experience.spec.ts` valida que Recommendations permanece neutra diante do novo resumo do Roteiro. Os ajustes são apenas de contrato de UI/navegação de teste e não alteram domínio, actions ou Providers.
