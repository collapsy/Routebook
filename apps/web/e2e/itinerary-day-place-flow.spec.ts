...
test("adiciona um Lugar ao Dia selecionado e retorna ao mesmo Dia", async ({ page }) => {
  ...
  await expect(addPlaceLink).toHaveAttribute("href", `/viagens/${trip.id}/lugares?dia=2026-08-23`);
  ...
  await expect(page).toHaveURL(
    new RegExp(`/viagens/${trip.id}/roteiro\\?atividadeCriada=1&dia=2026-08-23#dia-em-focoimport { expect, test } from "@playwright/test";
