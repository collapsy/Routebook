...
function normalizeAttributionUri(value: unknown): string | undefined {
...
function parseAuthorAttributions(value: unknown): readonly GoogleAuthorAttribution[] {
  ...
  return value.flatMap((entry) => {
    ...
    const uri = normalizeAttributionUri(candidate.uri);
...
function parsePhotoResources(value: unknown): readonly GooglePhotoResource[] {
  ...
  return value.flatMap((entry) => {
    ...
    const sourceUrl = normalizeGoogleMapsUri(candidate.googleMapsUri);
    return [
      {
        name,
        authorAttributions: parseAuthorAttributions(candidate.authorAttributions),
        ...(sourceUrl ? { googleMapsUri: sourceUrl } : {}),
      },
    ];
  });
}
...
export class GooglePlacePhotoAdapter {
  ...
  private async fetchDetails(
    ...
    if (!PLACE_ID_PATTERN.test(placeId)) {
      throw new Error("Google Place ID inválido para mídia.");
    }
    ...
    if (!response.ok) {
      throw new Error(`Google Place Details respondeu HTTP ${response.status}.`);
    }
  ...
  async findPreview(input: Readonly<{
    ...
    const photos = parsePhotoResources(details.photos);
    const photo = photos[0];
    if (!photo) return undefined;

    const mediaToken = encodeMediaToken(this.apiKey, {
  ...
  async fetchMedia(token: string): Promise<GooglePlacePhotoMedia | undefined> {
    ...
    const photos = parsePhotoResources(details.photos);
    const photo = photos[payload.photoIndex];
    if (!photo) return undefined;
    ...
    if (!response.ok) {
      throw new Error(`Google Place Photos respondeu HTTP ${response.status}.`);
    }
